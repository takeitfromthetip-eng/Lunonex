...
if (Test-Path "$fortheweebs\src\components") {
    $fortComponents = Get-ChildItem "$fortheweebs\src\components" -Recurse -File -ErrorAction SilentlyContinue | Select-Object Name, LastWriteTime, Length, FullName
}

if (Test-Path "$currentDir\src\components") {
    $lunoComponents = Get-ChildItem "$currentDir\src\components" -Recurse -File -ErrorAction SilentlyContinue | Select-Object Name, LastWriteTime, Length, FullName
}
...
# Find files in Fortheweebs that are newer or do not exist in lunonex
$uniqueOrNewer = @()
...
Write-Host ""
Write-Host "  Files to migrate from Fortheweebs: $($uniqueOrNewer.Count)" -ForegroundColor Cyan

# Step 2: Copy unique/newer files
if ($uniqueOrNewer.Count -gt 0) {
    Write-Host ""
    Write-Host "[2/7] Migrating unique/newer files..." -ForegroundColor Yellow
    ...
    Write-Host ""
    Write-Host "[2/7] No files to migrate - lunonex is up to date" -ForegroundColor Green
}

# Step 3: Create deletion manifest
Write-Host ""
Write-Host "[3/7] Creating deletion manifest..." -ForegroundColor Yellow
$manifest = @()
...
Write-Host ""
Write-Host "[4/7] Eliminating Fortheweebs from existence..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "[5/7] Deleting duplicate Desktop/Lunonex..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "[6/7] Deleting OneDrive duplicate..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "[7/7] Cleaning up garbage files..." -ForegroundColor Yellow
