...
router.post('/create', async (req, res) => {
  try {
    const result = await aiProxy.generate('cloud-storage', JSON.stringify(req.body));
...
router.get('/list', async (req, res) => {
  try {
    const result = await aiProxy.generate('cloud-storage-list', JSON.stringify(req.query));
