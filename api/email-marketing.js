...
router.post('/create', async (req, res) => {
  try {
    const result = await aiProxy.generate('email-marketing', JSON.stringify(req.body));
...
router.get('/list', async (req, res) => {
  try {
    const result = await aiProxy.generate('email-marketing-list', JSON.stringify(req.query));
